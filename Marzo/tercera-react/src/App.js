...
const imagenesDeLosGatos = gatos.map(gato => gato.img);

console.log(imagenesDeLosGatos);

const alumnas = ["Malu", "Lila", "Zhu"];

function App() {
	return (
		<div className="App">
			{imagenesDeLosGatos.map((imagen, i) => (
				<img key={i} alt="imagen de un gato" src={imagen} />
			))}
			{alumnas.map((alumna, i) => (
				<TarjetaDeAlumna key={i} nombreDeAlumna={alumna} />
			))}
		</div>
	);
}
